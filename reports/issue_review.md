...
目标：JiT-B/16、ImageNet 256×256、从头训练 200 epochs。论文对应 FID-50K 为 4.37。上游公开代码是 JAX/TPU 原作的 PyTorch/GPU 重实现；README 训练脚本经过 8×H200 测试。[官方仓库](https://github.com/LTH14/JiT)、[论文附录 A](https://arxiv.org/html/2511.13720v1#A1)。
...
| 200/600 epochs 的最佳 CFG 不同 | #56、#42 | 预注册 issue 参数；完整搜索论文三组 EMA × CFG 1.0–4.0，步长 0.1 |
...
- 新增第三组 EMA=0.9998，与论文一致；公开代码默认只维护两组。
- 每 40 epochs 做 8K 固定参数监测，不与论文正式 FID 比较；最终才执行完整搜索及 50K 评估。
...
- 50K 的主结果由 8K 选参决定；同时列出成功自述设置与 README 默认设置的诊断结果，不事后挑三者中最小 FID 当主结果。
